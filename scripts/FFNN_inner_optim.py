...
#       (unscaled) features while training on scaled ones. Fixed by adding
#       `X_val = scaler.transform(X_val)` immediately after the scaler fits.
#
# PERFORMANCE
#   [2] Duplicate idx_to_minhash dict — was built twice (line 277 & 385).
#       Removed the first construction; the second (post-shuffle) is correct.
#   [3] Exclusion-list membership checks inside the hot O(B×P) nested loop
#       used list `in` (O(n)). Converted exclude_bacts/phages/clusters to
#       sets before the loop for O(1) lookups.
#   [4] Entity-order string comparisons ("bact_first" / "phage_first") were
#       re-evaluated for every pair. Pre-resolved to a boolean `bact_first`.
#   [5] All-pairs prediction used scaler.transform(single_row) inside a tight
#       double loop. Replaced with a single batched matrix transform + batched
#       DataLoader inference pass — orders of magnitude faster for large grids.
#   [6] optimizer.zero_grad() → zero_grad(set_to_none=True) in both training
#       loops, reducing GPU memory overhead per step.
#   [7] DataLoaders now use num_workers=4 and pin_memory=True for async,
#       pinned-memory data transfer to the GPU.
#   [8] Removed redundant .to(device) calls on tensors already on device.
#
# CODE QUALITY
